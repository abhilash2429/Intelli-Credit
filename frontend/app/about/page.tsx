...
export default function AboutPage() {
  return (
    <div className="bg-ob-bg pt-[60px]">
      <div className="max-w-[900px] mx-auto py-20 px-4 md:px-8">
        {/* Page Header */}
        <p className="text-[10px] font-medium tracking-[0.16em] uppercase text-ob-text">
          About Intelli·Credit
        </p>
        <h1 className="font-display text-[42px] font-normal text-ob-text mt-3 leading-tight">
          Built for Indian credit officers
        </h1>
        <p className="text-[15px] text-ob-muted leading-[1.8] mt-4 max-w-[640px]">
        ...
        <div className="mt-12 border-l-2 border-ob-edge pl-6 space-y-5">
          <p className="text-[15px] text-ob-text leading-[1.85]">
        ...
        <div className="mt-16 bg-ob-surface border border-ob-edge rounded-[12px] p-8">
          <p className="text-[10px] font-medium tracking-[0.12em] uppercase text-ob-muted mb-6">Architecture Pipeline</p>
              ...
              return (
                <div key={stage.title} className="flex gap-4">
                  ...
                  <div className="flex flex-col items-center">
                    <div className="w-[30px] h-[30px] bg-ob-glass2 rounded-[6px] flex items-center justify-center text-ob-text text-[11px] font-mono font-medium flex-shrink-0">
                      {idx + 1}
                    </div>
                    {!isLast && <div className="w-px flex-1 bg-ob-edge ml-0" />}
                  </div>
                  {/* Right column: content */}
                  <div className="pb-5">
                    <p className="text-[13px] font-medium text-ob-text">{stage.title}</p>
                    <p className="text-[12px] text-ob-muted mt-0.5">{stage.desc}</p>
                  </div>
                </div>
              );
            })}
        ...
        {/* Tech Stack */}
        <div className="mt-12">
          <p className="text-[10px] font-medium tracking-[0.12em] uppercase text-ob-muted mb-4">Tech Stack</p>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {TECH_STACK.map((t) => (
              <div key={t.name} className="bg-ob-surface2 rounded-[8px] p-4">
                <p className="font-mono text-[13px] text-ob-text">{t.name}</p>
                <p className="text-[11px] text-ob-muted mt-1">{t.role}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mt-12">
          <h2 className="font-display text-[22px] text-ob-text">Built for Indian workflows</h2>
          <div className="flex flex-wrap gap-2 mt-4">
            {INDIA_CHIPS.map((chip) => (
              <span
                key={chip}
                className="bg-ob-glass2 text-ob-text rounded-full px-4 py-1.5 text-[12px] font-medium"
              >
                {chip}
              </span>
            ))}
        ...
        {/* Data Handling Note */}
        <div className="mt-8 bg-ob-surface2 border border-ob-edge rounded-[10px] p-5 flex gap-3">
          <svg className="w-5 h-5 text-ob-muted flex-shrink-0 mt-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
            <rect x="3" y="11" width="18" height="11" rx="2" ry="2" />
            <path d="M7 11V7a5 5 0 0 1 10 0v4" />
          </svg>
          <p className="text-[13px] text-ob-muted leading-[1.7]">
